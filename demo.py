...
def test_dense(dataset):
    ...
    """
    tffm = tffmclassifier.TFFMClassifier(factor_num=10, batch_size=1, print_step=100, input_type='dense')
    X = dataset[0]
    y = dataset[1]
    tffm.fit(X, y)
...
def test_sparse(dataset):
    """
        Unittest dense predict
    """
    tffm = tffmclassifier.TFFMClassifier(factor_num=10, batch_size=1, print_step=100, input_type='sparse')
...
def test_load_chkpt(dataset):
    ...
    tffm = tffmclassifier.TFFMClassifier(factor_num=10, batch_size=1, input_type='dense',
        chkpt_dir=chkpt_dir)
    X = dataset[0]
    y = dataset[1]
    tffm.fit(X, y)
    ...
    lr_load = tffmclassifier.TFFMClassifier(input_type='dense')
...
def test_export(dataset, input_type='dense'):
    ...
    tffm = tffmclassifier.TFFMClassifier(factor_num=10, batch_size=1, input_type=input_type)
    X = dataset[0]
...
def test_colind_model(X_colind, X_colval, y, feature_num):
    ...
    """
    model = tffmclassifier_column_indexed.TFFMClassifier(
...
def test_colind_load_chkpt(X_colind, X_colval, y, feature_num):
    ...
    print('train acc: {}'.format(accuracy_score(
        np.array(y_cid_tr), model.predict(np.array(X_ind_tr), np.array(X_val_tr)))))
    model_load = tffmclassifier_column_indexed.TFFMClassifier(
        feature_num=feature_num, # feature num must set
        l2_weight=0.01, learning_rate=1e-2,
